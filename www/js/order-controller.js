angular.module('starter.order-controller', [])
    ...
    .controller('CreateOrderCtrl', function ($scope, $state, $window, $stateParams, $ionicHistory, $ionicModal) {
        ...
        $scope.orderedProducts = [
            ...
            },
            {
                count: 1,
                price: 2288,
                name: '杭之吻毛呢外套女中长款加厚修身型呢子大衣女冬装显瘦韩范版毛领新款潮 1051皮粉色(带真毛领) M加棉加厚',
        ...
        $scope.userAddresses = ['A simple plugin that "sticks" an element to the top of the browser window while scrolling past ',
            ', always keeping it in view. This plugin works on multi-page sites, but has additional features for one-pager layouts.',
            'Please Visit the GitHub Page for installation instructions.'];

        $scope.closeSelectAddressDialog = function () {
            $scope.modal.hide();
        };

        $scope.openAddressListPage = function () {
            $scope.modal.show();
        };

        $scope.selectAddressForShipping = function () {
            $scope.modal.hide();
        };

        $ionicModal.fromTemplateUrl('templates/pay/address-list.html', {
            scope: $scope,
